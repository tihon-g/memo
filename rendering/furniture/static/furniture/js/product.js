...
function updateCamera(){
    camera.position.set(params['camera_lookat_x'], params['camera_pos_y'], params['camera_pos_z']);
    camera.lookAt(params['camera_lookat_x'], params['camera_lookat_y'], params['camera_lookat_z']);
...
function initGUI(canvas){
    ...
    gui.add( params, 'camera_pos_z', -25, 0 ).listen().step(0.05).onChange(updateCamera);
    gui.add( params, 'camera_pos_y', 0, 10 ).listen().step(0.01).onChange(updateCamera);
    gui.add( params, 'camera_lookat_x', -6, 6 ).listen().step(0.5).onChange(updateCamera);
    gui.add( params, 'camera_lookat_y', 0, 4 ).listen().step(0.01).onChange(updateCamera);
    gui.add( params, 'camera_lookat_z', -2, 2 ).listen().step(0.05).onChange(updateCamera);

   // gui.add( params, 'camera_lookat_y', 0, 3 ).step(0.1).onChange( function( value ){  updateCamera(); } );
...
//            models[model].parts.rims.push(
//                fModel.getObjectByName( 'rim_fl' ),
//                fModel.getObjectByName( 'rim_fr' ),
//                fModel.getObjectByName( 'rim_rr' ),
//                fModel.getObjectByName( 'rim_rl' ),
//                fModel.getObjectByName( 'trim' ),
//            models[model].parts.wheels.push(
//                 fModel.getObjectByName( 'wheel_fl' ),
//                 fModel.getObjectByName( 'wheel_fr' ),
//                 fModel.getObjectByName( 'wheel_rl' ),
//                 fModel.getObjectByName( 'wheel_rr' )

    if (model_ind in modelIDs){
        if (part_name != 'rims' || model_ind == 22){
            models[modelIDs[model_ind]].getObjectByName( part_name ).material = materialsLib[mat_id] ;
            render();
            }
        else console.log('rims on ferrari not changed now');
    }
...
//      for (let k=0; k < panels.length; k++){
//        if (k in modelIDs){
//            if (document.getElementById('model-' + k +'-rot-y').innerHTML.indexOf('stop')>=0)
//
//            //console.log(models[modelIDs[k]].rotation); //.quaternion
//        }
//      }
//    }
